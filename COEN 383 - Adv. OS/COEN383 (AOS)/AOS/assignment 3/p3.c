...
int h1_cust_customer,m1_cust_counter = 0,m2_cust_counter = 0,m3_cust_counter = 0, l1_cust_counter = 0,l2_cust_counter = 0,l3_cust_counter = 0,l4_cust_counter = 0,l5_cust_counter = 0,l6_cust_counter = 0;
...
void printCustomerQ(int N)
{
	...
	for(i = 0; i < NUM_QUEUE; ++i)
    {
    	...
    	for (j = 0; j < N; ++j)
    	{
    		...
    		printf(" C%02d:%d\t|", custQ[i].cust[j].custId, custQ[i].cust[j].arrival_time);
    	}
    	printf("\n");
    	printf("\n");
